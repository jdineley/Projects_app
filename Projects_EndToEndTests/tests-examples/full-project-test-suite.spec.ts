...
test.beforeAll(
  async ({
    ...
    // test("clear all e2e test artifacts", async ({ request }) => {
...
test.describe("require login tests", () => {
  ...
  test("test ms project edit percentage complete & finish date", async ({
    ...
    const inWorkUser = await projectDetailPage.getFirstInWorkUser();
  ...
  test("Create and populate Review", async ({
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await page
        .getByPlaceholder("New objective description")
        .fill(newObjective);
      if (i === 0) {
    ...
    for (const obj of reviewDataTracker) {
      ...
      await expect(
        page.getByText(objectiveName, { exact: true })
      ).toBeVisible();
      await page
